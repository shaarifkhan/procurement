...
export default class DynamicList extends Component {
  ...
  async createPDF(data) {
    ...
    var date =
      ...
      today.getSeconds();
    var mytable =
      "<html><body><h1>Procurement Report - Dated = " +
      date +
      '</h1><table cellpadding="5" cellspacing="5"><thead><td>Item Name</td><td>Item Quantity</td><td>Item Rate</td><td>Total Amount</td><td>Market Rate</td><td>Comments</td></thead><tbody>';
    for (var i = 0; i < data.length; i++) {
      mytable += "<tr>";
    ...
    mytable +=
      "<td></td><td></td><td> Total Amount: Rs. " +
      totalAmount +
          ...
          <View style={styles.contact}>
            ...
            <View style={{ flexDirection: "row" }}>
              <Text style={styles.phone}>
                Weight : {rowData.weight} {rowData.unit} |
              </Text>
              <Text style={styles.phone}> Amount: {rowData.amount} Rs. |</Text>
              <Text style={styles.phone}>
                {" "}
                Rate: {rowData.rate} / {rowData.unit}{" "}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              ...
              <Text style={styles.phone}>
                {" "}
                Market Rate: {rowData.marketrate} Rs.{" "}
              </Text>
            </View>
          </View>
          <TouchableOpacity
            style={styles.deleteWrapper}
            onPress={() => this._deleteItem(rowData.uid, rowData.amount)}
          >
  ...
  addData(buttonIndex) {
    if (buttonIndex == 0) {
      ...
      fetch(
        "https://script.google.com/macros/s/AKfycbzDVJNUDnQfn7T4LmAVqnsFgZThZPUEh_gXb7IIhFrGDFjOnEYf/exec",
        // "http://192.168.1.102:3000/sheet",
        {
          mode: "no-cors",
          method: "post",
          headers: {
      ...
      console.log(JSON.stringify(formData));
      fetch(
        "https://script.google.com/macros/s/AKfycbyh0kRFQt6AOBG7NsnXFttfGF6fO27NoH7SbqKCmTdOEY7FPSQ/exec",
        // "http://192.168.1.102:3000/sheet",
        {
          mode: "no-cors",
          method: "post",
      ...
      console.log(JSON.stringify(formData));
      fetch(
        "https://script.google.com/macros/s/AKfycbzPGhucYwFLDoTeLc7G-_6mGLKScUTsPA2T8c4G/exec",
        // "http://192.168.1.102:3000/sheet",
        {
          mode: "no-cors",
          method: "post",
      ...
      console.log(JSON.stringify(formData));
      fetch(
        "https://script.google.com/macros/s/AKfycbxnbhMEIItGQ_rn227_x8TPBS7hQPQz_tDM_66mdA/exec",
        // "http://192.168.1.102:3000/sheet",
        {
          mode: "no-cors",
          method: "post",
